...
describe("Given I am connected as an Admin and I am on Dashboard Page", () => {
    describe("When bill data is passed to DashboardUI", () => {
        test("Then, it should render them in the page", () => {
    ...
    describe("When bill has missing comment and file name", () => {
        test("Then, it should render fallback messages", () => {
            const html = DashboardFormUI(billWithMissingFields);
            document.body.innerHTML = html;
            expect(
                screen.getAllByText("Aucun commentaire disponible."),
            ).toHaveLength(2);
            expect(
